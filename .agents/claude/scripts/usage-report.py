...
Reads Claude Code's session logs and prints a breakdown by model, project, and
day (with cache-hit % and heuristic callouts). No network calls — everything
stays on disk.

Supported platforms: macOS, Linux, Windows 10+

Log locations (searched in order):
  macOS / Linux : ~/.claude/projects/
  Windows       : %USERPROFILE%\\.claude\\projects\\
                  %LOCALAPPDATA%\\Claude\\projects\\   (fallback)
...
def _default_projects_dir() -> Path:
    """Return the most likely Claude Code projects directory for this OS."""
    # Primary: ~/.claude/projects  (works on all platforms via pathlib)
    primary = Path.home() / ".claude" / "projects"
    if primary.exists():
        return primary

    # Windows fallback: %LOCALAPPDATA%\Claude\projects
    if platform.system() == "Windows":
        local_app = os.environ.get("LOCALAPPDATA", "")
        if local_app:
            win_path = Path(local_app) / "Claude" / "projects"
            if win_path.exists():
                return win_path

    # Return primary even if it doesn't exist (error message will guide user)
    return primary
...
def main() -> int:
    ...
    ap.add_argument("--projects-dir", type=str, default=None,
    ...
    if not projects_dir.exists():
        print(
            f"No Claude Code logs found at {projects_dir}\n"
            f"  macOS/Linux : {Path.home() / '.claude' / 'projects'}\n"
            f"  Windows     : {Path.home() / '.claude' / 'projects'} "
            f"or %LOCALAPPDATA%\\Claude\\projects",
            file=sys.stderr,
        )
        return 1
